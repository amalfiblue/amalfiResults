...
def load_sample_candidates() -> bool:
    ...
    try:
        ...
        if count > 0:
            logger.info(f"Found {count} existing candidates for Warringah, skipping sample data load")
            conn.close()
            return True
        ...
        cursor.execute("SELECT COUNT(*) FROM tcp_candidates WHERE electorate = 'Warringah'")
        ...
        logger.info(f"Successfully loaded {len(warringah_candidates)} sample candidates for Warringah")
        return True
    except Exception as e:
